...
void check_args(char *ip, char * port, int sc_mode, int bandwidth, 
			int streams , double duration, int pack_size, int flag){
...
int convert_rate_bits(char * bandwidth){
	int b;
	char c ='\0';

	sscanf(bandwidth, "%d%c", &b, &c);
...
int convert_rate_bytes(char * bytes){
	int b;
	char c ='\0';

	sscanf(bytes, "%d%c", &b, &c);
...
int main(int argc, char *argv[]){
	...
	int bandwidth;			/*bandwidth in bits/sec*/
	int p_streams;			/*number of parallel data streams*/	
